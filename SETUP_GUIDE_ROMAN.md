...
## 4.5) Frontend AI configuration (SRS Chatbot)
- Project root mein `.env.local.example` hai. Usko copy karke `.env.local` banao:
  ```powershell
  cd D:\path\to\project
  Copy-Item .env.local.example .env.local
  notepad .env.local
  ```
- `.env.local` mein `OPENAI_API_KEY` set karo (yeh key browser mein expose nahi hoti; Next.js server route use karta hai).
