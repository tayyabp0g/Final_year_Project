...
function sanitizeMermaidForExport(raw) {
  ...
  for (let line of lines) {
    line = String(line ?? "").replace(/\t/g, "  ").trimEnd();
    ...
    // Remove trailing ASCII separators and other junk
    ...
    // Pipe characters inside node labels often break Mermaid (e.g., process[|process])
  ...
  if (!/^(flowchart|graph)\s+/i.test(first.trim())) cleaned.unshift("flowchart LR");
  return cleaned.join("\n").trim();
}
...
export async function POST(req) {
  try {
    ...
    const drawCodeBlock = (code, { isMermaid } = {}) => {
      ...
      const codeText = String(code || "").replace(/\t/g, "  ").replace(/\r\n/g, "\n");
    ...
    const drawMermaidDiagram = async (mermaidCode) => {
      try {
        const sanitized = sanitizeMermaidForExport(mermaidCode);
        if (!sanitized) throw new Error("Empty diagram");
        const png = await renderMermaidPng(sanitized);
